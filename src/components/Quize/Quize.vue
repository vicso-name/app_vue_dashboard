<template>
    <!-- QUIZ SECTION -->
    <div class="lesson-exercise-builder">

    <h2 class="lesson-details__title">Quiz Section</h2>
...
<VueDraggableNext v-model="localExercises" @end="onDragEnd">
      <div v-for="(exercise, index) in localExercises" :key="exercise.id">
        ...
        <div v-else>
          <!-- Display Mode -->
          <p><strong>{{ exercise.content }}</strong></p>
          <p>{{ exercise.tip }}</p>
...
export default {
  ...
  data() {
    return {
      ...
      // Удалено дублирование exerciseCorrectAnswer
      exerciseType: '',
      exerciseContent: '',
      exerciseTip: '',
      exerciseChoices: '',
